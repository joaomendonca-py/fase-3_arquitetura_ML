"""
Glue Job - IMDb Title Principals ETL
Tech Challenge Fase 3 - Seguindo padrão das Fases 1 e 2

Job Glue para processar dados IMDb title principals (elenco principal):
...
def process_imdb_title_principals():
    """Processa dados IMDb title principals seguindo arquitetura medalhão"""
    ...
    df_trusted = clean_title_principals_data(df_raw)
...
def clean_title_principals_data(df_raw: DataFrame) -> DataFrame:
    """Limpa e valida dados de title principals"""
...
def create_ml_features(df_trusted: DataFrame) -> DataFrame:
    ...
    # Feature: posição no elenco (billing order)
    df_features = df_features.withColumn("billing_position",
        when(col("ordering") == 1, "lead")
        .when(col("ordering") <= 3, "main_cast")
        .when(col("ordering") <= 10, "supporting")
        .otherwise("minor")
    )
    
    # Feature: é ator/atriz
    df_features = df_features.withColumn("is_actor", 
        (col("category").isin(["actor", "actress"])).cast("integer"))
    
    # Feature: é diretor
    df_features = df_features.withColumn("is_director", 
        (col("category") == "director").cast("integer"))
    
    # Feature: é produtor
    df_features = df_features.withColumn("is_producer", 
        (col("category") == "producer").cast("integer"))
    
    # Feature: tem personagem especificado
    df_features = df_features.withColumn("has_character", 
                                       col("characters").isNotNull().cast("integer"))
    
    # Feature: tem job específico
    df_features = df_features.withColumn("has_job", 
                                       col("job").isNotNull().cast("integer"))
    
    # Feature: é elenco principal (top 5)
    df_features = df_features.withColumn("is_main_cast", 
                                       (col("ordering") <= 5).cast("integer"))
    
    # Feature: categoria de profissional
    df_features = df_features.withColumn("role_category",
        when(col("category").isin(["actor", "actress"]), "acting")
        .when(col("category") == "director", "directing") 
        .when(col("category") == "producer", "producing")
        .when(col("category") == "writer", "writing")
        .when(col("category") == "composer", "music")
        .otherwise("other")
    )
...
def create_table_definition(glue_client, database: str, table: str, location: str, layer: str):
    ...
    if layer == "trusted":
        ...
        columns = [
            ...
            {'Name': 'job', 'Type': 'string'},
            {'Name': 'characters', 'Type': 'string'},
            {'Name': 'billing_position', 'Type': 'string'},
            {'Name': 'is_actor', 'Type': 'int'},
            {'Name': 'is_director', 'Type': 'int'},
            {'Name': 'is_producer', 'Type': 'int'},
            {'Name': 'has_character', 'Type': 'int'},
            {'Name': 'has_job', 'Type': 'int'},
            {'Name': 'is_main_cast', 'Type': 'int'},
